...
DEFAULT_OUTPUT_DIR = Path(__file__).resolve().parent / "output" / "smallcap_amount_shock_event"
DEFAULT_CACHE_DIR = Path(__file__).resolve().parent / "cache" / "smallcap_amount_shock_event"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the small-cap amount-shock event-driven backtest.")
    parser.add_argument("--start-date", default="2020-01-01")
    parser.add_argument("--end-date", default="2026-05-21")
    parser.add_argument("--initial-cash", type=float, default=1_000_000)
    parser.add_argument("--top-k", type=int, default=4, help="每日最多新开候选数。")
    ...
    parser.add_argument("--ret-keep-groups", default="3,4,5")
    parser.add_argument("--min-research-ret-10d", type=float, default=0.12)
    parser.add_argument("--ret-top-pct", type=float, default=0.40)
    parser.add_argument("--selection-sort", default="amount_expand_desc", choices=sorted(EVENT_SELECTION_SORTS))
    ...
    parser.add_argument("--cache-version", default="amount_shock_event_v1")
...
def main() -> None:
    ...
    log_event(
        "info",
        "smallcap_amount_shock_event_run_start",
    ...
    strategy = SmallCapAmountShockEventStrategy(
        top_k=args.top_k,
        hold_days=args.hold_days,
    ...
    strategy.event_signal_frame.to_csv(output_dir / "event_signal_table.csv", index=False, encoding="utf-8-sig")
    strategy.event_daily_window_features.to_csv(output_dir / "event_daily_window_features.csv", index=False, encoding="utf-8-sig")
    report.export(
